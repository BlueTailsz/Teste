...
namespace SADES.Util
{
    class MiscControl
    {
        ...
        public static bool isValidTarget(Obj_AI_Base target)
        {
            if (target.IsDead || target.IsInvulnerable || !target.IsVisible || target.Health == 0 || !target.IsTargetable)
            {
                return false;
            }
            return true;
        ...
        public static float calcularPorcentagem(Obj_AI_Base t1)
        {

            return (t1.Health / t1.MaxHealth) * 100;
        }

        public static bool GetBool(string name)
        {
            return GameControl.Config.Item(name).GetValue<bool>();
        }

        public static void PrintChat(string msg)
        {
            Game.PrintChat("<font color='#07c7dd'>SADES: </font> <font color='#FFFFFF'>" + msg + "</font>");
        }
